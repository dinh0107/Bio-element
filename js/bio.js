...
(function () {
    ...
    function initStickyHeader() {
        var header = document.querySelector('.site-header');
        var navPanel = document.querySelector('.site-header__nav-panel');
        if (!header || !navPanel) return;

        var stickyOffset = 0;

        function setStickyOffset() {
            header.classList.remove('is-scrolled');
            stickyOffset = navPanel.getBoundingClientRect().top + window.scrollY;
            onScroll();
        }

        function onScroll() {
            header.classList.toggle('is-scrolled', window.scrollY >= stickyOffset);
        }
    ...
    function initTherapyTabs() {
        ...
        swapImg.className = 'home-therapies__featured-img home-therapies__featured-img--next';
        swapImg.alt = '';
        swapImg.setAttribute('aria-hidden', 'true');
        img.insertAdjacentElement('afterend', swapImg);
        ...
        function startImageSwap(nextSrc) {
            isSwapping = true;
            window.clearTimeout(swapTimer);
            swapImg.classList.remove('is-visible');
            swapImg.src = nextSrc;
            swapImg.dataset.src = nextSrc;

            window.requestAnimationFrame(function () {
                window.requestAnimationFrame(function () {
                    swapImg.classList.add('is-visible');
                });
            });

            swapTimer = window.setTimeout(finishImageSwap, 560);
        }

        function setFeaturedImage(nextSrc) {
            if (!nextSrc) return;
            if (!isSwapping && img.getAttribute('src') === nextSrc) return;
            if (queuedSrc === nextSrc || swapImg.dataset.src === nextSrc) return;

            queuedSrc = nextSrc;

            preloadImage(nextSrc, function () {
                if (queuedSrc !== nextSrc || isSwapping) return;
                queuedSrc = null;
                startImageSwap(nextSrc);
            });
        }
        ...
        function finishImageSwap() {
            if (!isSwapping || !swapImg.classList.contains('is-visible')) return;

            window.clearTimeout(swapTimer);
            var finishedSrc = swapImg.dataset.src;
            if (finishedSrc) img.src = finishedSrc;

            swapImg.classList.remove('is-visible');
            swapImg.removeAttribute('src');
            delete swapImg.dataset.src;
            isSwapping = false;

            if (queuedSrc && queuedSrc !== finishedSrc) {
                var nextSrc = queuedSrc;
                queuedSrc = null;
                setFeaturedImage(nextSrc);
            }
        }

        swapImg.addEventListener('transitionend', function (e) {
        ...
        tabs.forEach(function (tab) {
            if (tab.dataset.image) preloadImage(tab.dataset.image);
